...
gbif.obs.observers <- gbif %>%
  group_by(City45km, year) %>%
  summarise(unique_observers = n_distinct(identifiedBy), .groups = "drop")
...
cities.test <- cities.test %>%
  ...
  mutate(
    n.obs45km = replace_na(n.obs45km, 0),
    unique_observers = replace_na(unique_observers, 0)
  )
