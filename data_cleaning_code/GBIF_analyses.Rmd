...
### Abundance values
```{r}
# Abundance for Megachilidae
gbif.obs.meg <- gbif %>%
  filter(family == "Megachilidae") %>%
  group_by(City45km, year) %>%
  summarise(megachilidae = n(), .groups = 'drop')

# Abundance for all bees EXCEPT Megachilidae
gbif.obs.nonmeg <- gbif %>%
  filter(family != "Megachilidae") %>%
  group_by(City45km, year) %>%
  summarise(non_megachilidae = n(), .groups = 'drop')

# Merge into cities.test and calculate totals + proportions
cities.test <- cities.test %>%
  ...
  left_join(gbif.obs.meg, by = c("City" = "City45km", "year")) %>%
  left_join(gbif.obs.nonmeg, by = c("City" = "City45km", "year")) %>%
  mutate(
    megachilidae = replace_na(megachilidae, 0),
    non_megachilidae = replace_na(non_megachilidae, 0),
    n.obs45km = megachilidae + non_megachilidae,   # keep original naming
    prop_megachilidae = if_else(
      n.obs45km > 0,
      megachilidae / n.obs45km,
      NA_real_
    )
  )
