...
feature.vars <- c("Temp","Prec","imperv45km","Population","non_megachilidae","tcc45km","area_km2","Median_HH_Income","BA_Population")
city.data <- city.data %>%
  rename(unit.no = ID)
...
dat <- scdata(df = city.data,
              ...
              cov.adj = list('Temp' = c(),
                             ...
                             'non_megachilidae' = c("constant","trend"),
                             'tcc45km' = c(),
                             'area_km2' = c(),
                             'Median_HH_Income' = c(),
                             'BA_Population' = c()))
...
(decline.2023 <- ((303.394- 279)/303.394)*100)
(decline.2022 <- ((268.740-212)/268.740)*100) 
```
...
scpi.plot <- ggplot(scpi.data, aes(x = year, y = nobs45km, color = Treatment)) +
  ...
  geom_point()+
    geom_point(aes(x = 2022, y = 268.740, color = "#c443b2", group = 1))+
        geom_errorbar(aes(x = 2022, ymin = 265.655, ymax = 341.675, color = "#c443b2"), width = 0.25)+
    geom_point(aes(x = 2023, y = 303.394, color = "#c443b2", group = 1))+
    geom_errorbar(aes(x = 2023, ymin = 297.900, ymax = 375.552, color = "#c443b2"), width = 0.25)+
...
gbifcity_out <- city.data %>%
  ...
  synthetic_control(outcome = non_megachilidae, # outcome
...
baci.model <- lm(non_megachilidae ~ Treated * Time, data = city.data.baci)
...
baci.plot <- ggplot(city.data.baci, aes(x = year, y = non_megachilidae, color = City)) +
...
#Running the Before- After Control Impact model
#baci.model <- feols(nobs45km ~ Treated * Time, data = city.data.baci, cluster = ~City)
baci.model.2 <- lm(non_megachilidae ~ Treated * Time, data = city.data.baci.2)
...
baci.plot.2 <- ggplot(city.data.baci.2, aes(x = year, y = non_megachilidae, color = City)) +
    geom_line(linewidth = 1, aes(linetype = City))+
  geom_point()+
  scale_linetype_manual(values=c(3,1))+
  scale_color_manual(values=c('#D55E00','darkgrey'))+
...
its.model <- gls(non_megachilidae ~ year + Treated + Time_since, data = city.data.its, correlation= corARMA(p=1, q=1, form = ~ year), method = "ML")
summary(its.model)
...
its.counter = gls(non_megachilidae ~ year, data = city.data.its.2, correlation= corARMA(p=1, q=1, form = ~ year),method="ML")
...
```{r}
#nference with subgaussian bounds   
#                  Treated Synthetic Left Bound Right Bound
#Philadelphia.2022     212   268.740    265.655     341.675
#Philadelphia.2023     279   303.394    297.900     375.552
method <- "   SC"
estimate <- (279 - 303.394) 
lwrCI <- (279 - 297.900)
uprCI <- (279 - 375.552)
