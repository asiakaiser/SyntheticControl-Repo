...
* Creating a data frame with observation values from all donor cities equally weighted in average. 
```{r}
#Observed outcomes
...
did.trendplot <- ggplot(city.data.did, aes(x = year, y = n.obs45km, color = City)) +
    ...
    geom_ribbon(aes(ymin = lwr, ymax = upr, fill = City), alpha = 0.5)+
  geom_point()+
...
its.plot <- ggplot(city.data.its,
         aes(year,n.obs45km))+
  geom_ribbon(aes(ymin = counter.predictions - (1.96*counter.se), ymax = counter.predictions + (1.96*counter.se)), fill = '#D55E00', alpha = 0.5)+
  geom_line(aes(year,counter.predictions),color="red",lty=2)+
  geom_ribbon(aes(ymin = its.predictions - (1.96*its.se), ymax = its.predictions + (1.96*its.se)), fill = '#56B4E9', alpha = 0.5)+
  geom_line(aes(year,its.predictions),color="black",lty=1)+
