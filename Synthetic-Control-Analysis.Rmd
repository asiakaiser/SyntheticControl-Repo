...
feature.vars <- c("Temp","Prec","imperv45km","Population","nobs45km","tcc45km","area_km2","Median_HH_Income","unique_observers")
city.data <- city.data %>%
  rename(unit.no = ID)
...
(decline.2023 <- ((349.839- 299)/349.839)*100)
(decline.2022 <- ((298.470-238)/298.470)*100) 
```
...
scpi.plot <- ggplot(scpi.data, aes(x = year, y = nobs45km, color = Treatment)) +
  ...
  geom_point()+
    geom_point(aes(x = 2022, y = 298.470, color = "#c443b2", group = 1))+
        geom_errorbar(aes(x = 2022, ymin = 297.368, ymax = 332.898, color = "#c443b2"), width = 0.25)+
    geom_point(aes(x = 2023, y = 349.839, color = "#c443b2", group = 1))+
    geom_errorbar(aes(x = 2023, ymin = 348.081, ymax = 383.700, color = "#c443b2"), width = 0.25)+
...
ba.plot <- ggplot(city.data.ba, aes(x = year, y = n.obs45km)) +
  ...
  geom_point(color = "darkgrey")+
    ...
    geom_segment(aes(x = 2020, y = ba.model$coefficients[1], xend = 2022, yend =  ( ba.model$coefficients[1] + ba.model$coefficients[2]), colour = "#8B0000"))+
  geom_vline(xintercept = 2021, linetype="dashed")+
