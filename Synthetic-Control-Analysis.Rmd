...
city.data <- city.data |>
    mutate(
        Treatment = ifelse(City == "Philadelphia" &  year > 2021,1,0)
    )
```
...
not converging
```{r}
cov.var <- c("area_km2", "Temp", "Prec", "tcc45km", "imperv45km")
match.out <- c("n.obs45km", "Population")

sea1 <- microsynth(city.data, 
                   idvar="City", timevar="year", intvar="Treatment", 
                   start.pre=2010, end.pre=2021, end.post=2023,
                   match.out = FALSE, match.covar = FALSE,
                   match.out.min= match.out, match.covar.min=cov.var, 
                   result.var= FALSE,
                   period = 1,
                   test="twosided",
...
```{r}
placebos <- gbifcity_out %>% plot_placebos(prune = TRUE)+
  Asia_Theme +
  labs(title = NULL, subtitle = NULL,
        x = "Year",
        y = "(Estimated - Observed) Bee observations")
...
gbifcity_out %>% grab_synthetic_control()%>%
  ...
  mutate(percent_estimate= (causal_estimate/(synth_y + real_y))*100)%>%
  head
```
...
lwrCI <- c(NA,confint(did.model)[4],confint(its.model)[3]) #replace SCM CI
uprCI <- c(NA,confint(did.model)[8],confint(its.model)[7]) #replace SCM CI
